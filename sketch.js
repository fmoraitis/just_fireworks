...
let gunSound;
let heardAGunSound;
let s1; // keep the seconds 0- 59
let s2;

let newCenter = [];


let previous_vidaActiveZone;
...
function preload() {
  ...
  theShader = loadShader('vertShader.vert', 'fireworks.frag');
  gunSound = loadSound('gun.mp3');
  //aTexture=createGraphics(w, h, [renderer]);
}
...
function setup() {
  ...
  //aTexture = createGraphics(windowWidth, windowHeight, WEBGL);
...
function draw() {
  ...
  s1 = second();
  //if ((abs(s1 - s2)) >= 5 && NUM_EXPLOSIONS >= 2.0) { // if time passed is more than 5 sec
  if ((abs(s1 - s2)) >= 2 && gunSound.isPlaying()==false ) { // if time passed is more than 5 sec
    NUM_EXPLOSIONS += 1.0;
    if (NUM_EXPLOSIONS >= 7) NUM_EXPLOSIONS = 7
    s2 = s1;
   // gunSound.play();
  ...
  //ambientLight(100);
   if (gunSound.isPlaying()==true){heardAGunSound=true}
   else{heardAGunSound=false}
  // here we're using setUniform() to send our uniform values to the shader
  theShader.setUniform("u_resolution", [windowWidth, windowHeight]);
  theShader.setUniform("u_time", millis() / 1000.0);
  //theShader.setUniform("u_center", [newCenter[0] * windowWidth, map(newCenter[1] * windowHeight, 0, windowHeight, windowHeight, 0)]);
  //theShader.setUniform("u_center", [mouseX, map(mouseY, 0, windowHeight, windowHeight, 0)]);
  theShader.setUniform("u_NUM_EXPLOSIONS", NUM_EXPLOSIONS);
  theShader.setUniform("u_heardAGunSound", heardAGunSound);

  shader(theShader);
  //shader(theShader);
  //aTexture.shader(theShader);
...
function onActiveZoneChange(_vidaActiveZone) {
  ...
  */
  ...
  if (_vidaActiveZone.id === 0 && _vidaActiveZone.isMovementDetectedFlag === true) {
    ...
    if (NUM_EXPLOSIONS >= 7) NUM_EXPLOSIONS = 7 //hard limit also the same value in the fragment shader
    console.log(
      'zone: ' + _vidaActiveZone.id +
      ' normX: ' + _vidaActiveZone.normX
    );

  }

  // newCenter[0]=lerp(_vidaActiveZone.normX,previous_vidaActiveZone.normX,0.05)
  // newCenter[1]=lerp(_vidaActiveZone.normY,previous_vidaActiveZone.normY,0.05)
  // previous_vidaActiveZone=_vidaActiveZone
